...
void BarnesHutParticleSystem::reset()
{
	...
	else if(parameters.model == disk_model){
		diskModel(h_mass, h_x, h_y, h_vx, h_vy, h_ax, h_ay, numParticles);
	}
    else if(parameters.model == csv_model){
        csvModel(parameters.modelPath, h_mass, h_x, h_y, h_vx, h_vy, h_ax, h_ay, numParticles);
    }
...
void BarnesHutParticleSystem::csvModel(std::string csvPath, float *mass, float *x, float *y, float *x_vel, float *y_vel, float *x_acc, float *y_acc, int n) {
    // Read in our CSV input data
    FILE* data = fopen(csvPath.c_str(), "r");
    Body* initialBodies = (Body*) malloc(sizeof(Body)  * n);
    readInput(data, initialBodies, (size_t) n);
    fclose(data);

    // Copy CSV into the initial inputs
    for (int i = 0; i < n; i++) {
        mass[i] = initialBodies[i].mass;
        x[i] = initialBodies[i].pos.x;
        y[i] = initialBodies[i].pos.y;
        x_vel[i] = initialBodies[i].vel.x;
        y_vel[i] = initialBodies[i].vel.y;
        x_acc[i] = 0.0f;
        y_acc[i] = 0.0f;
    }
